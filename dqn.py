...
class ReplayMemory(object):
    ...
    def sample(self, bsz):
        ...
        return map(lambda x: Variable(torch.cat(x, 0).cpu()), zip(*batch))
...
class StatePrep(object):
    ...
    def __init__(self, prepfn, size):
        ...
        self.transform = T.Compose([
            ...
            T.Scale(size, interpolation=Image.BOX),
            T.ToTensor()])
...
def dqn(E, args, work_dir):
    ...
    def act(s, eps):
        P.fill(eps / nA)
        q, argq = Q(Variable(s.cpu(), volatile=True)).data.cpu().max(1)
        P[argq.item()] += 1 - eps
    ...
    for e in range(args.max_episodes):
        ...
        while not done:
            ...
            if ep_len % args.learn_freq == 0:
                ...
                psp =T(bns).detach().max(1)[0].squeeze(0)
